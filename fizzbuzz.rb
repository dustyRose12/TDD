...
class Fizzbuzz
  def output(number)
    ...
    return "FizzBuzz" if number % 15 == 0 #(same thing as div by 3 and 5 duh)    
...
RSpec.describe Fizzbuzz do
  describe '#output' do
    #this is the right syntax so it's available for all it statements but it resets before running each one so it's a fresh test
